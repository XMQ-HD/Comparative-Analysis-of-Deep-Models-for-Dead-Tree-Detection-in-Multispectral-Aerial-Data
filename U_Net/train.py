import os
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"  # Avoid OpenMP multi-threading conflicts (for PyTorch on Windows)
import torch
import torch.nn as nn
from torch.utils.data import DataLoader
import matplotlib
matplotlib.use('Agg')  # Enable saving plots without display
...
def tversky_loss(pred, target, alpha=0.7, beta=0.3, epsilon=1e-6):
    """Tversky loss, more flexible than Dice"""
    pred = torch.sigmoid(pred)
    tp = (pred * target).sum()
...
def main():
    args = get_args()
    # Load sample triplets (RGB, NIR, mask) from pkl file
    ...
    # Save folder naming by config
    folder_name = f"unetpp_loss_{args.loss}_size_{args.img_size}_opt_{args.optimizer}_bs_{args.batch_size}_lr_{args.lr}"
    ...
    print("✅ Training complete. Best model saved to:", os.path.join(save_folder, "best_model.pth"))
